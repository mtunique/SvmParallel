...
#include "svm.h"
#include <mpi.h>
int mpi_id, mpi_size, *mpi_queue, *mpi_info, *mpi_start, *mpi_len;
MPI_Status sta;
                   ...
                   SolutionInfo* si, int shrinking)
{
    ...
    mpi_start = Malloc(int, mpi_size);
    mpi_len   = Malloc(int, mpi_size);
...
public:
    ...
    Qfloat *get_Q(int i, int len) const
    {
        ...
        else
        {
            ...
            data = Malloc(Qfloat, new_len);
            for (int i = 0; i< mpi_size; ++i) {
                
                MPI_Recv(data, new_len, MPI_INT, MPI_ANY_SOURCE, 1, MPI_COMM_WORLD, &sta);
                for (int j = mpi_start[sta.MPI_SOURCE]; j <= mpi_start[sta.MPI_SOURCE] + mpi_len[sta.MPI_SOURCE]; ++j) {
                    re_data[j] = data[j - mpi_start[sta.MPI_SOURCE]];
                }
